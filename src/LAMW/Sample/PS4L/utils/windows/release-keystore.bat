set JAVA_HOME=C:\lamw_manager\LAMW\jdk\zulu-default
set PATH=%JAVA_HOME%\bin;%PATH%
set JAVA_TOOL_OPTIONS=-Duser.language=en
cd D:\files\pascalscada_v0\src\LAMW\Sample\PS4L
keytool -genkey -v -keystore ps4l-release.keystore -alias ps4l.keyalias -keyalg RSA -keysize 2048 -validity 10000 < D:\files\pascalscada_v0\src\LAMW\Sample\PS4L\keytool_input.txt
:Error
echo off
cls
echo.
echo Signature file created previously, remember that if you delete this file and it was uploaded to Google Play, you will not be able to upload another app without this signature.
echo.
pause
